...
var Aways = function(depList) {
    ...
    this.addHelp({
        'away': 'Type away while on the decks, and I\'ll guard your spot for a ' + 'minute so you can refresh. Type away while on queue, and I\'ll ' + 'save your spot for 10 minutes no matter what!',
        'back': 'If you marked yourself as away while on queue, type back to ' + 'let me know you\'ve returned.'
    });
    ...
    this.installHandlers = function() {
        ...
        bot.on('speak', function(data) {
            ...
            if ((reData = data.text.match(/^ *\/?away *$/i))) {
                if (utils.isDj(id)) {
                    self.addAway(id, data.name, __isDj);
                }
                else if (queue.getQueuePosition(id) !== false) {
                    self.addAway(id, data.name, __isQueued);
                }
                else {
                    bot.speak('You don\'t have a spot for me to hold, ' + name + ' :P');
                    return;
                }
