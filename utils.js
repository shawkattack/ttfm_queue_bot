...
var Utils = function(depList) {
    ...
    this.addHelp({
        'oog': 'Type oog to mark the current song as \'out-of-genre\'. ' + 'If enough people join you, I\'ll remove the problem :)'
    });
    ...
    this.installHandlers = function() {
        ...
        bot.on('endsong', function(data) {
            ...
            bot.speak(data.room.metadata.current_song.metadata.song + " got :arrow_up: " + data.room.metadata.upvotes + " :arrow_down: " + data.room.metadata.downvotes + " :hearts: " + __snagCounter);
        ...
        bot.on('pmmed', function(data) {
            ...
            if (self.isMod(sender)) {
                ...
                if ((reData = data.text.match(/^ *\/?mod +(\S.*?) *$/i))) {
                    user = self.getUserByTag(reData[1]);
                    if (!user) {
                ...
                else if ((reData = data.text.match(/^ *\/?lock *mods *$/i)) || (reData = data.text.match(/^ *\/?set +mod *lock +on *$/i))) {
                    self.lockMods();
                    bot.pm('Okie dokie, I\'ll lock down mods and demods', sender);
                }

                else if ((reData = data.text.match(/^ *\/?unlock *mods *$/i)) || (reData = data.text.match(/^ *\/?set +mod *lock +off *$/i))) {
                    self.unlockMods();
    ...
    this.getNumSpots = function() {
        ...
        return self.getMaxDjs() - self.getNumDjs() - numAwayDjs;
    };
