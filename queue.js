...
var Queue = function(depList) {
    ...
    this.addHelp({
        ...
        'myspot': 'Type myspot and I\'ll tell you how many people are in ' + 'front of you in the queue.'
    });
        ...
        vips.on('speak', function(data) {
            ...
            if (utils.isMod(data.userid)) {
                if ((reData = data.text.match(/^ *\/?q([+-][0-9]+) +(\S.*?) *$/i))) {
                    ...
                    result = self.moveInQueue(uData.id, amt);
                    var newSpot = -1;
                    msg = '';
                    switch (result) {
                    case __isDj:
                    ...
                    bot.speak(msg);
                    if (__callInfo !== null && __callInfo.id === uData.id && newSpot > oldSpot) {
                        self.callNext();
                    }
                }
            }
            ...
            if ((reData = data.text.match(/^ *\/?q\+ *$/i))) {
                ...
                case __isDj:
                    ...
                    break;
                case __isAway:
                    msg = 'I\'m holding your spot, ' + tag + '. ' + 'Quit fooling around and hop up!';
                    break;
                case __isQueued:
                ...
                case __didKick:
                    return;
                default:
                    if (self.getRealQueuePosition(data.userid) + 1 <= utils.getNumSpots()) {
                        msg = 'Hop on up, ' + tag + ' ;)';
                    }
                    else {
